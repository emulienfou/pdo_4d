...
$db->exec('CREATE TABLE test(id int NOT NULL, classtype int, val VARCHAR(10),  PRIMARY KEY(id))');
...
$stmt = $db->prepare('SELECT classtypes.name, test.id AS id, test.val AS val FROM test, classtypes WHERE test.classtype=classtypes.id');
