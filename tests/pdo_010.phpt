...
$db->exec('CREATE TABLE classtypes(id int NOT NULL, name VARCHAR(10),  PRIMARY KEY(id))');
...
$db->exec('INSERT INTO classtypes VALUES(3, null)'); 
$db->exec('CREATE TABLE test(id int NOT NULL, classtype int, val VARCHAR(10), grp VARCHAR(10),  PRIMARY KEY(id))');
...
$stmt = $db->prepare('SELECT classtypes.name, test.grp AS grp, test.id AS id, test.val AS val FROM test, classtypes WHERE test.classtype=classtypes.id');
